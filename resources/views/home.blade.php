...
@section('content')
    ...
    <section id="home" class="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden">
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('assets/img/hero.webp') }}" loading="lazy" alt="خلفية ديكور" class="w-full h-full object-cover">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($projects as $index => $project)
                        <div class="group relative overflow-hidden rounded-lg shadow-md h-80" data-aos="fade-up"
                            data-aos-delay="{{ $index * 100 }}">
                            @if ($project->main_image)
                                <img src="{{ asset('storage/' . $project->main_image) }}" alt="{{ $project->title }}" loading="lazy"
                                    class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            @else
                                <div class="w-full h-full bg-gray-200 flex items-center justify-center">
                                    <span class="text-gray-400">لا توجد صورة</span>
