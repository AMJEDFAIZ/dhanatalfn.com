  ...
  <header id="header"
      ...
      <div class="container mx-auto px-4 py-2 flex justify-between items-center">
          ...
          <a href="{{ route('home') }}" class="flex items-center gap-3 group min-w-0">
              ...
              <img src="{{ isset($settings['site_logo']) ? asset('storage/' . $settings['site_logo']) : asset('assets/img/logo.PNG') }}"
                  ...
                  class="h-14 sm:h-16 md:h-20 object-contain group-hover:scale-105 transition-transform duration-300 shrink-0">
              @endif
              <div class="hidden sm:block min-w-0">
                  <p class="text-primary font-bold text-base md:text-lg leading-tight truncate">الفن الحديث</p>
                  <p class="text-sm md:text-base text-accent font-bold truncate">للدهانات والديكور</p>
                  @if (isset($settings['phone']))
                  <p class="text-sm md:text-base text-accent font-bold" dir="ltr">{{ $settings['phone'] }}</p>
                  @endif
              </div>
          </a>
          @php
          ...
          <!-- Mobile Menu Button -->
          <button id="mobile-menu-btn" type="button" aria-label="فتح القائمة" aria-controls="mobile-menu"
              aria-expanded="false" class="lg:hidden text-primary text-2xl focus:outline-none">
      ...
      <div id="mobile-menu" aria-hidden="true"
          ...
          <!-- Close Button -->
          <button id="close-menu-btn" type="button" aria-label="إغلاق القائمة"
